...
partial class NotificationApiTest
{
    [Fact]
    public static async Task SubscribeAsync_BotInfoGetResultIsFailure_ExpectUnknownFailure()
    {
        ...
        var api = new NotificationApi(mockDataverseApi.Object, mockBotApi.Object);

        var actual = await api.SubscribeAsync(SomeSubscribeInput, TestContext.Current.CancellationToken);
    ...
    public static async Task SubscribeAsync_BotInfoGetResultIsSuccess_ExpectSearchForUserCalledOnce()
    {
        ...
        var api = new NotificationApi(mockDataverseApi.Object, mockBotApi.Object);
        _ = await api.SubscribeAsync(input, TestContext.Current.CancellationToken);
    ...
    [Theory]
    [MemberData(nameof(NotificationApiSource.InputInvalidTestData), MemberType = typeof(NotificationApiSource))]
        ...
        NotificationSubscribeIn input, Failure<NotificationSubscribeFailureCode> expected)
    {
        ...
        var actual = await api.SubscribeAsync(input, TestContext.Current.CancellationToken);

        Assert.StrictEqual(expected, actual);
    }

    [Theory]
    [MemberData(nameof(NotificationApiSource.InputValidTestData), MemberType = typeof(NotificationApiSource))]
